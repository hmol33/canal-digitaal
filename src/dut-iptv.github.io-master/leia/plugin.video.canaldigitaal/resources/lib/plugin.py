...
import datetime, json, pytz, random, string, sys, time, uuid, xbmc, xbmcplugin
...
def get_live_channels(addon=False, retry=True):
    ...
    if rows:
        ...
        for row in rows:
            ...
            path = plugin.url_for(func_or_url=play_video, type='channel', channel=channeldata['channel_id'], id=channeldata['channel_id'], _is_live=True)
            playable = True
...
def process_replaytv_list(data, start=0):
    ...
    for row in sorted(data):
        ...
        if check_key(currow, 'cn') and '18+' in currow['cn']:
            if _debug_mode:
                log.debug('Skipping XXX')
