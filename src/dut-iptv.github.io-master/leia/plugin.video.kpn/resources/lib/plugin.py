...
_img_size = settings.get(key='_img_size')
        ...
        for row in rows:
            ...
            if (len(unicode(channeldata['channel_id'])) > 0):
                channels.append({
                    ...
                    'playable': playable,
                    'context': [
                        (_.START_BEGINNING, 'RunPlugin({context_url})'.format(context_url=plugin.url_for(func_or_url=play_video, type='channel', channel=channeldata['channel_id'], id=channeldata['asset_id'], from_beginning=True)), ),
                    ],
                })
...
def process_replaytv_content(data, day=0, start=0):
    ...
    for row in data:
        ...
        if check_key(currow, 'i'):
            program_image = currow['i'].replace(_img_size, '1920x1080')
            program_image_large = currow['i'].replace(_img_size, '1920x1080')
...
def process_vod_content(data, start=0, search=None, type=None):
    ...
    for row in data:
        ...
        if check_key(currow, 'image'):
            program_image = currow['image'].replace(_img_size, '1920x1080')
            program_image_large = currow['image'].replace(_img_size, '1920x1080')
